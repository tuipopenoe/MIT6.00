...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    for k in kvals:
        for j in range(len(points)):
            if j % 5 == 0:
                holdouts.append(points[j])
            else:
                training.append(points[j])
        # Find error of training set
        total_training_error = calc_total_error(training, k, cutoff,
            type(training[0]))
        total_holdouts_error = calc_total_error(holdouts, k, cutoff,
            type(holdouts[0]))

        # TODO: Graph Error vs K.
        # TODO: Graph ratio of holdout error over training error

        del holdouts[:]
        del training[:]

def calc_total_error(points, k, cutoff, pointType):
    clusters = kmeans(points, k, cutoff, pointType)
    total_error = 0
    # Loop through clusters
    for i in clusters:
        # Loop through points in cluster
        for j in i:
            total_error += i.distance(j.getCentroid())
    return pow(total_error, 2)
